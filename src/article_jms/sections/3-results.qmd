...
Une image Pléiade telle qu'elle est livrée par l'IGN contient 2000\*2000 pixels. Elle est trop volumineuse pour être prise en entrée d’un réseau de neurones avec les capacités usuelles disponibles de mémoire vive du processeur graphique GPU (Graphics Processing Unit). Pour prévenir cela, l'ensemble des images des mosaïques seront découpées en plusieurs sous-images de 250\*250 pixels, appelées des patchs. Comme un pixel sur une image Pléiades équivaut à 0.5\*0.5m², alors un patch a une superficie de 15 625m². La @fig-surface montre le nombre de patchs par mosaïque envoyés pour l'entraînement, et également la superficie représentée en km².

::: {#fig-surface .cell-output-display}
|  | train (nombre) | train (superficie)| test (nombre) | test (superficie)|
|:---------------------|:----------:|:--------:|:------:|:--------:|
| Guadeloupe  (2022)   |      | ** **  |      |  ** **  | |
| Martinique  (2022)   |      | ** **  |      |  ** **  |
| Mayotte     (2023)   | 22 875     | **357**  |   841  |  **13**  |
:::

<!-- mc cp -r s3/projet-slums-detection/data-preprocessed/patchs/COSIA/segmentation/PLEIADES/MARTINIQUE/2022/ .
cd 250/train/
ls -1 | wc -l -->
...
avec :  
$$
\text{- } c^* : \text{classe vraie du pixel } i,
$$
$$
\text{- } p_{i,c^*} : \text{probabilité prédite pour cette classe},
$$
$$
\text{- } w_c : \text{poids associé à la classe}
$$
...
<!-- faire avec excalidraw -->
::: {#fig-pred .cell-output-display}
<table>
<tr>
<td><img src="../img/pleiades_mayotte_2023.png" width="30%"><br><em>Pleiades © CNES_2023</em></td>
<td><img src="../img/cosia_multiclasse.png" width="30%"><br><em>Couverture du sol COSIA</em></td>
<td><img src="../img/predictions_test_2023.png" width="30%"><br><em>Prédictions</em></td>
<td><img src="../img/../img/legende_cosia.png" width="30%"><br><em>Légende COSIA</em></td>
</tr>
</table>

**Figure : Comparaison visuelle sur une zone de test de Mayotte (2023)**
:::
