...
domains: # utiliser la nomenclature du site (menu déroulant) 
  - Science des données – Apprentissage statistique (machine learning, deep learning, réseaux de neurones, ...)
resume: |
...
format:
  jms2025-typst:
    logo:
      location:
        path: src/article_jms/_extensions/InseeFrLab/jms2025/head_jms2025.png
        x: center
        y: top
      width: 160mm
    mainfont: "tex gyre heros" # peut être modifié si nécessaire
    footer: 15^e^ édition des journées de méthodologie statistique de l'Insee (JMS 2025)
    section-numbering: 1.1.1.
    keep-typ: true
    # Options fusionnées de typst
    toc: true
    toc-indent: auto
    columns: 1
    bibliography: biblio.bib
...
`include("sections/1-introduction.qmd")`{=typst}

#page

# Méthodologie

`include("sections/2-methodo.qmd")`{=typst}

#page

# Résultats

`include("sections/3-results.qmd")`{=typst}

#page

# Application : contôle de la qualité des enquêtes cartographiques du recensement de la population à Mayotte {#sec-use_case}

`include("sections/4-use_case.qmd")`{=typst}
